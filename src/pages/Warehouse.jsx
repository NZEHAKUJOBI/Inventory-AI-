    ...
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
        {[
          ...
          { label: 'Inventory Accuracy',  value: '98.3%',  sub: 'Avg across all',     color: colors.green, icon: TrendingUp    },
          { label: 'Total Locations',     value: '1,230',  sub: 'Storage slots',      color: colors.yellow, icon: MapPin        },
          { label: 'Utilization Rate',    value: '78%',    sub: 'Average fill rate',  color: colors.purple, icon: Package       },
        ].map(s => (
          <Card key={s.label} style={{ flex: 1, minWidth: 170 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: colors.textSecondary }}>{s.sub}</div>
              </div>
        ...
        {warehouses.map(w => (
          <Card key={w.id} style={{ flex: 1, minWidth: 230 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
              <div>
                <div style={{ fontSize: 14, fontWeight: 700, color: colors.textPrimary }}>{w.name}</div>
                <div style={{ fontSize: 11, color: colors.textSecondary, marginTop: 2 }}>{w.items} items &nbsp;·&nbsp; {fmt(w.value)}</div>
              </div>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 5 }}>
                ...
                <span style={{ color: colors.green, fontWeight: 600 }}>{w.accuracy}%</span>
              </div>
              <div style={{ background: colors.border, borderRadius: 4, height: 6 }}>
                <div style={{ width: `${w.accuracy}%`, background: colors.green, borderRadius: 4, height: '100%' }} />
              </div>
            </div>
          </Card>
        ))}
      ...
      <Card style={{ padding: 0 }}>
        <div style={{ padding: '14px 20px', borderBottom: `1px solid ${colors.border}` }}>
          <h3 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: colors.textPrimary }}>Warehouse Inventory Breakdown</h3>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            ...
            <tbody>
              {warehouses.map((w, idx) => (
                <tr key={w.id} style={{ borderBottom: `1px solid ${colors.border}`, background: idx%2===0?'transparent':colors.rowAlt }}>
                  <td style={{ padding: '12px 20px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ width: 10, height: 10, borderRadius: '50%', background: w.color, display: 'inline-block' }} />
                      <span style={{ color: colors.textPrimary, fontWeight: 500 }}>{w.name}</span>
                    </div>
                  </td>
                  <td style={{ padding: '12px 20px', color: colors.textPrimary }}>{w.items.toLocaleString()}</td>
                  <td style={{ padding: '12px 20px', color: colors.textPrimary }}>{fmt(w.value)}</td>
                  <td style={{ padding: '12px 20px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <div style={{ flex: 1, background: colors.border, borderRadius: 3, height: 5 }}>
                  ...
                  <td style={{ padding: '12px 20px', color: colors.green, fontWeight: 600 }}>{w.accuracy}%</td>
                  <td style={{ padding: '12px 20px' }}>
                    <span style={{ padding: '3px 10px', borderRadius: 6, fontSize: 11, fontWeight: 600, background: colors.greenLight, color: colors.green }}>Active</span>
                  </td>
                </tr>
              ))}
            </tbody>
